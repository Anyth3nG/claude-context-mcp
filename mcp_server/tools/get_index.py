...
The intended first call of a session, and the reason get_context should no longer
...
CALL THIS FIRST when you need stored context and don't know what's there. It is far cheaper than get_context (tens of tokens rather than thousands), needs no query, and tells you exactly which follow-up is worth making: get_context narrowed to one category or slot, get_context on the whole project, search_context for history.

OPEN A SESSION WITH detail="projects". That returns the table of contents — one line per project with its size and shape, no per-slot detail — for roughly a tenth of the full form. It is the cheapest possible way to learn whether this store holds anything relevant to what you are about to do, and the answer is usually "yes, in one specific project", which is then a single get_context away.
