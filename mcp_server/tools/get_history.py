...
DESCRIPTION = """Get everything the store remembers under ONE key — every archived version of the slot AND every entry appended to that key, newest first, with the current value alongside for comparison.
...
Returns the live value (or null if the slot was archived outright), then each archived version with its text, when it was written, when it stopped being current, and any reason recorded at the time — and separately `appended`, the entries add_update filed under this key: things that happened alongside the slot rather than versions of it.
...
def get_history(
    category: Annotated[
        ...
        Field(
            ...
            "general); get_index lists every category, including ones created later."
        ),
    ],
    project: Annotated[
        Optional[str],
    ...
    if not result["versions"] and not result["appended"]:
        result["note"] = (
            f"No history under {result['slot']} — it has either never been "
            "rewritten, or was only ever patched in small pieces that did not "
            "trigger an archive, and nothing was appended to it. The current value "
            "is the whole story."
        ) if result["current"] else (
            f"Nothing at {result['slot']}: no live summary, no archived versions, "
            "nothing appended. Check get_index for the keys this project actually has."
        )
    return result
