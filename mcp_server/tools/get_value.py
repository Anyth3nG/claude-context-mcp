...
ALSO use this before calling change_update. change_update REPLACES a category's entire contents, so you need the current text in front of you to fold your change into it; sending only the part that changed destroys the rest.
...
def get_value(
    ...
    store = get_store()
    found = store.get_summary(project=project, category=category)
    if found is None:
        return {
            "found": False,
