...
DESCRIPTION = """Look up prior context — past decisions, architecture, tech stack, preferences, facts, or goals — that isn't already visible in this conversation. This store is cross-machine and cross-client (claude.ai and Claude Code both write to it), so it can hold things captured on a different device or the other client that no local, single-machine memory would know about.
...
Returns up to 5 matching entries (capped at 10), each with its text (truncated to ~800 characters) and metadata (project, category, type, source, timestamp)."""
...
def search_context(
    ...
    store = get_store()
    raw = store.search(query=query, project=project, category=category)
